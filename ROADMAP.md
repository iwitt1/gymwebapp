...
- ~~**Garmin CSV importer** (Sept 2026) — Settings box that parses a Garmin Connect activities export into `run_logs`: quote-aware CSV parsing, Garmin activity-type mapping, non-cardio rows (strength/yoga) skipped, duplicate-safe on re-paste, avg HR + ascent packed into the note. Added `hike`, `sup`, `hiit` cardio types. Built because the cardio log sat empty for two months while 26 hours of training existed in Garmin — manual double-entry was never going to work.~~
- ~~**Phase II transition + 3-day restructure** (Sept 2026, Week 15) — Phase I gate passed (4 of 5; 14 weeks with zero knee pain flags). Default schedule cut 4 → 3 days to match real frequency (Weeks 12–14 were 4/2/2), with Wednesday resolving to Upper A or Upper B automatically by program-week parity via an `'upper'` sentinel in `DEFAULT_DAY_MAP` + `resolveDayAssignment()`. Lower B moved to Friday and rebuilt as a hard Core-5 after failing 3 consecutive times. New exercise `e13` Skater Bound replaces the goblet/DB-deadlift power block; `c8` wall sit promoted out of Optional; `d11` retired to Optional; Upper B shoulder volume cut ~30%; `b6`/`b9` moved ahead of the cable block; `a9` regressed to two-leg. Tue/Thu home-screen copy now prompts conditioning.~~
